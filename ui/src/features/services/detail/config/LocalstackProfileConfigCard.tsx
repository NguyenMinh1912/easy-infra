...
interface TextField {
  key: "host" | "port" | "region" | "version" | "services";
  label: string;
  placeholder: string;
  type?: string;
  hint?: string;
...
const TEXT_FIELDS: readonly TextField[] = [
  ...
  { key: "port", label: "Port", placeholder: "4566", type: "number" },
  { key: "region", label: "Region", placeholder: "us-east-1" },
  { key: "version", label: "Version", placeholder: "latest" },
  {
    key: "services",
    label: "Services",
    placeholder: "s3, sqs, sns",
    hint: "Comma-separated AWS services to emulate.",
  },
];

/** Keys this card owns; rows with any other key are preserved untouched. */
const KNOWN_KEYS = new Set<string>(TEXT_FIELDS.map((f) => f.key));

/** The editable values this card surfaces, all held as strings. */
interface LocalstackFields {
  host: string;
  port: string;
  region: string;
  version: string;
...
/**
 * Tailored profile config editor for localstack. Instead of the generic
 * editable key/value rows, it shows a form with named fields — connection
 * details, image version and the AWS services to emulate — plus a health-check
 * button, so a user fills in information without having to know (or be able to
 * mistype) the underlying config keys. Unknown keys are preserved untouched,
 * mirroring the minio card and the "don't special-case service names"
 * convention.
 *
 ...
 * coerces these string values (a numeric port), so the rows stay a plain string
 * map.
 */
export function LocalstackProfileConfigCard({
  name,
  profileName,
  rows,
  onChange,
  ...
  const setField = <K extends keyof LocalstackFields>(
    key: K,
    value: LocalstackFields[K],
  ) => {
...
function readState(rows: ConfigRow[]): LocalstackFields {
 ...
 * any unknown rows (e.g. the cleanable flag).
 */
function fieldsToRows(
  fields: LocalstackFields,
  prevRows: ConfigRow[],
): ConfigRow[] {
  ...
  for (const field of TEXT_FIELDS) {
    ...
    if (value.trim() !== "") known.push({ key: field.key, value });
  }
