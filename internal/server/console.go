...
// relatedRequest selects the rows on the far side of a relation: the related
// table and the equality predicates that join it to the originating row. A null
// filter value matches NULL. Values travel as text so the database coerces them
// to each column's type.
type relatedRequest struct {
	Schema  string            `json:"schema"`
	Table   string            `json:"table"`
	Filters []relatedFilterIn `json:"filters"`
}

// relatedFilterIn is one column-equals-value predicate of a relatedRequest.
type relatedFilterIn struct {
	Column string  `json:"column"`
	Value  *string `json:"value"`
}

// handleRelatedRows fetches the rows reachable through a foreign-key relation.
// Like a console query, the result (rows or an error) comes back on a 200 so
// the UI renders it the same way — including its own editability and relations,
// so the user can keep exploring.
func (s *Server) handleRelatedRows(w http.ResponseWriter, r *http.Request) {
	...
	browser, ok := svc.(service.RelationBrowser)
	if !ok {
		writeError(w, http.StatusBadRequest, fmt.Sprintf("service %q does not support relations", r.PathValue("service")))
		return
	}

	filters := make([]service.RelationFilter, len(req.Filters))
	for i, f := range req.Filters {
		filters[i] = service.RelationFilter{Column: f.Column, Value: f.Value}
	}

	ctx, cancel := context.WithTimeout(r.Context(), queryTimeout)
	defer cancel()
	start := time.Now()
	res, err := browser.RelatedRows(ctx, spec, service.RelatedQuery{
		Schema: req.Schema, Table: req.Table, Filters: filters,
	})
	elapsed := time.Since(start).Milliseconds()
	if err != nil {
		writeJSON(w, http.StatusOK, queryResponse{
